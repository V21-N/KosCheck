...
@section('content')
{{-- KosCheck+ Upgrade Banner (if not premium) --}}
@if(!auth()->user()->isPremiumUser())
<div class="mb-6 p-4 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-2xl">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-gradient-to-r from-orange-500 to-amber-500 flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                </svg>
            </div>
            <div>
                <p class="text-sm font-bold text-text">Upgrade ke KosCheck+</p>
                <p class="text-xs text-text-muted">Dapatkan badge Verified untuk properti Anda dan naikkan visibilitas.</p>
            </div>
        </div>
        <a href="{{ route('koscheck-plus.index') }}" class="px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white text-xs font-bold rounded-lg transition-all whitespace-nowrap">
            Upgrade Sekarang
        </a>
    </div>
</div>
        ...
        <div class="card bg-white shadow-sm border border-border-light overflow-hidden flex flex-col md:flex-row rounded-3xl relative pr-6" data-hover="lift" data-reveal>
            <div class="relative w-full md:w-72 h-56 md:h-auto flex-shrink-0">
                ...
                <div class="absolute top-3 left-3 flex gap-2 flex-wrap">
                    ...
                    @if(auth()->user()->isPremiumUser())
                        <span class="badge bg-gradient-to-r from-orange-500 to-amber-500 text-white text-[0.65rem] border border-orange-500 shadow-sm flex items-center gap-1">
