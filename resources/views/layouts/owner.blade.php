    ...
    <aside class="owner-sidebar hidden lg:block">
        <div class="flex h-full flex-col">
            ...
            <nav class="px-4 flex-1 min-h-0 overflow-y-auto">
                ...
                <a href="{{ route('koscheck-plus.index') }}" class="owner-nav-item {{ request()->routeIs('koscheck-plus.*') ? 'active' : '' }} @if(!auth()->user()->isPremiumUser()) bg-gradient-to-r from-orange-500 to-amber-500 text-white hover:from-orange-600 hover:to-amber-600 @endif">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                    <span>KosCheck+</span>
                    @if(!auth()->user()->isPremiumUser())
                    <span class="ml-auto bg-white text-orange-600 text-[0.6rem] font-bold px-2 py-0.5 rounded-full">UPGRADE</span>
                    @endif
                </a>
