                    ...
                    <div class="p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div class="flex items-center gap-3">
                            @if($listing->photos->first())
                                <img src="{{ str_starts_with($listing->photos->first()->url, 'http') ? $listing->photos->first()->url : asset('storage/' . $listing->photos->first()->url) }}"
                                     alt="{{ $listing->name }}"
        ...
        <div class="space-y-4">
            @foreach($topKos as $index => $kos)
                <div class="flex items-center gap-4 p-4 rounded-xl border border-border-light hover:bg-bg transition-colors">
                    ...
                    @if($kos->photos->first())
                        <img src="{{ str_starts_with($kos->photos->first()->url, 'http') ? $kos->photos->first()->url : asset('storage/' . $kos->photos->first()->url) }}"
                             alt="{{ $kos->name }}"
                             class="w-12 h-12 rounded-xl object-cover">
                    @else
                        <div class="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center">
